...
static struct msm_hsusb_platform_data msm_hsusb_pdata = {
	...
	.usb_uart_switch	= config_rider_usb_uart_gpios,
...
static void config_flashlight_gpios(void)
{
	...
	pm8058_gpio_config(RIDER_TORCH_SET1, &flashlight_cfg);
	pm8058_gpio_config(RIDER_TORCH_SET2, &flashlight_cfg);
}
...
static int Rider_s5k3h2yx_vreg_on(void)
{
	...
	pr_err("[CAM] sensor_power_enable(\"8058_l10\", 2.85V) == %d\n", rc);
	...
	pr_err("[CAM] sensor_power_enable(\"8058_l15\", 2.8V) == %d\n", rc);
	...
	if (system_rev == 0x80)
	{
		rc = camera_sensor_power_enable("8901_l0", 1200000);
		pr_err("[CAM] sensor_power_enable(\"8901_l0\", 1.2V) == %d\n", rc);
	}
	else
	{
		rc = camera_sensor_power_enable("8058_l24", 1200000);
		pr_err("[CAM] sensor_power_enable(\"8058_l24\", 1.2V) == %d\n", rc);
	}
...
static int Rider_s5k3h2yx_vreg_off(void)
{
	...
	pr_err("[CAM] sensor_power_disable(\"8058_l10\") == %d\n", rc);
	...
	if (system_rev == 0x80)
	{
		rc = camera_sensor_power_disable("8901_l0");
		pr_err("[CAM] sensor_power_disable(\"8901_l0\") == %d\n", rc);
	}
	else
	{
	...
	pr_err("[CAM] sensor_power_disable(\"8058_l15\") == %d\n", rc);
...
static void rider_config_camera_on_gpios(void)
{
...
static struct msm_serial_hs_platform_data msm_uart_dm1_pdata = {
	.rx_wakeup_irq = MSM_GPIO_TO_INT(RIDER_GPIO_BT_HOST_WAKE),
...
static struct msm_serial_hs_platform_data msm_uart3_pdata = {
	.rx_wakeup_irq = MSM_GPIO_TO_INT(RIDER_GPIO_WIMAX_UART_SOUT),
	.inject_rx_on_wakeup = 0,
	.cpu_lock_supported = 1,
};
#endif
...
static struct rpm_vreg_pdata rpm_vreg_init_pdata[RPM_VREG_ID_MAX] = {
	...
	RPM_VREG_INIT_LDO(PM8058_L12, 0, 1, 0, 3000000, 3000000, LDO150HMIN, 0),
	...
	RPM_VREG_INIT_LDO(PM8058_L22, 0, 0, 0, 1200000, 1200000, LDO300HMIN, 0), /* On till S4B on then off */
	RPM_VREG_INIT_LDO(PM8058_L23, 0, 0, 0, 1800000, 1800000, LDO300HMIN, 0), /* On till S3A on then off */
...
static int pm8058_gpios_init(void)
{
	...
	gpio_set_value(PM8058_GPIO_PM_TO_SYS(20), 0);
	return 0;
}
...
static struct tpa2051d3_platform_data tpa2051d3_pdata = {
	.gpio_tpa2051_spk_en = RIDER_AUD_SPK_ENO,
...
static uint32_t msm_spi_gpio[] = {
	GPIO_CFG(RIDER_SPI_DO,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(RIDER_SPI_DI,  0, GPIO_CFG_INPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA),
	GPIO_CFG(RIDER_SPI_CS,  0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
	GPIO_CFG(RIDER_SPI_CLK, 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),
};
...
#ifdef CONFIG_USB_G_ANDROID
static void rider_add_usb_devices(void)
{
	printk("%s\n", __func__);
	...
	msm_otg_pdata.idgnd_gpio = RIDER_GPIO_USB_ID;
	msm_device_otg.dev.platform_data = &msm_otg_pdata;
